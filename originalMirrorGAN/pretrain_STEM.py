...
def build_models(vocab_len):
    print(type(vocab_len))
    print(vocab_len)
    text_encoder = RNN_ENCODER(vocab_len, nhidden=cfg.TEXT.EMBEDDING_DIM)
...
def train(dataloader, cnn_model, rnn_model, batch_size,
    ...
    for step, data in enumerate(tqdm(dataloader)):
        ...
        if step % UPDATE_INTERVAL == 0:
            ...
            # attention Maps
            img_set, _ = \
                build_super_images(imgs.cpu(), captions, dataloader.ixtoword, attn_maps, att_sze)
...
def main():
    ...
    # Train ##############################################################
    text_encoder, image_encoder, labels, start_epoch = build_models(dataset.n_words)
        ...
        for epoch in range(start_epoch, cfg.TRAIN.MAX_EPOCH):
            ...
            if cfg.TRAIN.FLAG:
                count, batch_w_losses, batch_s_losses = \
                    train(dataloader, image_encoder, text_encoder, batch_size, labels, optimizer, epoch, image_dir)
